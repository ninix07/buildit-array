...
// Helper function to check if two arrays are compatible for pointwise
// operations and return the resultant size
...
template <typename T> struct barray_expr {
  virtual const builder::dyn_var<T>
  get_value_at(std::vector<builder::dyn_var<int> *> indices) const {
    return 0;
  }
  virtual const std::vector<int> get_expr_size(void) const { return {}; }
};

template <typename T> struct barray_expr_const;
template <typename T> struct barray_expr_array;
...
template <typename T> struct barray {
  ...
  barray(builder::dyn_var<T *> arr, std::vector<int> sizes)
      : m_arr(arr), m_sizes(sizes) {}

  // Constructor to allocate a new barray with provided size
  barray(std::vector<int> sizes) { allocate(sizes); }
  ...
  void to_host() {
    if (current_storage == DEVICE_HOST)
      return;
  ...
  void induce_loop_at(std::vector<builder::dyn_var<int> *> indices,
                      const barray_expr<T> &rhs) {
  ...
  void operator=(const barray &other) { *this = barray_expr_array<T>(other); }

  // Function to convert multi-dimensional index into a single index
  // based on the sizes
  builder::dyn_var<int>
  compute_flat_index(std::vector<builder::dyn_var<int> *> indices,
    ...
    return *(indices[index]) +
           (int)m_sizes[index] * compute_flat_index(indices, index - 1);
  }
  ...
  void destroy() { runtime::free(m_arr); }
};

// RHS class for constants in the generated code
template <typename T> struct barray_expr_const : public barray_expr<T> {
  T m_value;
  barray_expr_const(const T &value) : m_value(value) {}

  const builder::dyn_var<T>
  get_value_at(std::vector<builder::dyn_var<int> *> indices) const {
    // Always return the same constant value
    return m_value;
  }

  const std::vector<int> get_expr_size(void) const { return {}; }
};

// RHS class for holding array operands
template <typename T> struct barray_expr_array : public barray_expr<T> {
  ...
  get_value_at(std::vector<builder::dyn_var<int> *> indices) const {
    ...
    if (m_array.is_constant) {
      ...
      if (current_device == DEVICE_HOST)
        ...
        return const_cast<builder::dyn_var<T *> &>(
            m_array.m_device_arr)[m_array.compute_flat_index(indices)];
    }
  }

  const std::vector<int> get_expr_size(void) const { return m_array.m_sizes; }
};

// Non leaf RHS expr classes

// Class for storing addition of two expressions
template <typename T> struct barray_expr_add : public barray_expr<T> {
  const struct barray_expr<T> &expr1;
  const struct barray_expr<T> &expr2;

  std::vector<int> computed_sizes;

  barray_expr_add(const struct barray_expr<T> &expr1,
                  const struct barray_expr<T> &expr2)
      : expr1(expr1), expr2(expr2) {
    computed_sizes =
        match_expr_sizes(expr1.get_expr_size(), expr2.get_expr_size());
  }

  const std::vector<int> get_expr_size(void) const { return computed_sizes; }
...
template <typename T> struct barray_expr_mul : public barray_expr<T> {
  const struct barray_expr<T> &expr1;
  const struct barray_expr<T> &expr2;

  std::vector<int> computed_sizes;

  barray_expr_mul(const struct barray_expr<T> &expr1,
                  const struct barray_expr<T> &expr2)
      : expr1(expr1), expr2(expr2) {
    computed_sizes =
        match_expr_sizes(expr1.get_expr_size(), expr2.get_expr_size());
...
template <typename T> struct barray_expr_cross : public barray_expr<T> {
  const struct barray_expr<T> &expr1;
  const struct barray_expr<T> &expr2;

  std::vector<int> computed_sizes;

  barray_expr_cross(const struct barray_expr<T> &expr1,
                    const struct barray_expr<T> &expr2)
      : expr1(expr1), expr2(expr2) {
    std::vector<int> size1 = expr1.get_expr_size();
    std::vector<int> size2 = expr2.get_expr_size();
    assert(size1.size() == 2 && size2.size() == 2 &&
           "Cross can only be performed on 2 dimensional arrays");
